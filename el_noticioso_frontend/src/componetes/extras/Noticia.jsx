...
export function Noticia({ id, titulo, resumen, imagen, autor, fecha, modo = "normal" ,onEliminar}) {
  ...
  async function agregarFavorito() {
    ...
    try {
      ...
      const responseUsuario = await UsuarioServicio.buscarIdUsuario(usuario.correoElectronico);
      const idUsuario = responseUsuario.data;

      const favorito = { idNoticia: id, idUsuario };
      const response = await FavoritoServicio.agregarFavorito(favorito);
  ...
  async function eliminarFavorito() {
    try {
      ...
      await FavoritoServicio.eliminarFavorito({ data: { idNoticia: id, idUsuario } });
